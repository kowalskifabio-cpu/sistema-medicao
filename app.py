            ...
            with st.container(border=True):
                ...
                if c1.button(f"🔍 Detalhar Boletim {con['ctt']}", key=f"btn_det_{cid}", use_container_width=True):
                    if not med_ctt.empty:
                        ...
                        rel['Data Limite'] = rel.get('data_fim_item', con['data_fim'])
...
# --- 5. ITENS ---
    ...
    if not df_c.empty:
        df_c = df_c[df_c['status'] == 'Ativo']
        if st.session_state.usuario != "ADM": df_c = df_c[df_c['gestor'] == st.session_state.usuario]
        ...
        if not df_c.empty:
            ...
            if not df_i.empty:
                ...
                st.divider()
                tot_l = i_f['vlr_unit'].apply(safe_float).sum(); v_con = safe_float(row_ctt['valor_contrato'])
                with st.container(border=True):
                    st.subheader("💰 Resumo Financeiro")
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Total Lançado", formatar_real(tot_l)); c2.metric("Valor Contrato", formatar_real(v_con))
                    diff = v_con - tot_l
...
# --- 7. KANBAN ---
    ...
    if not df_c.empty:
        ...
        if not df_m.empty:
            ...
            else: m_f = m_f[m_f['item_id'].isin(df_i[df_i['contract_id'].isin(df_c['contract_id'])]['item_id'])]
        cols = st.columns(4)
...
# --- 8. RELATÓRIO ---
    ...
    if not df_c.empty:
        ...
        if st.session_state.usuario != "ADM": df_at = df_at[df_at['gestor'] == st.session_state.usuario]
        if not df_at.empty:
            ...
            with c2:
                if not med_ctt.empty:
                    ...
                    with pd.ExcelWriter(output, engine='openpyxl') as writer:
                        df_header = pd.DataFrame([["BOLETIM", ""], ["Obra:", f"{con.get('ctr', '-')} - {con.get('cliente', 'Cliente')}"], ["Gestor:", con.get('gestor', '-')], ["Data:", datetime.now().strftime('%d/%m/%Y')]])
...
# --- 9. CTRs CONCLUÍDAS (ADICIONADA FUNÇÃO DE REABERTURA) ---
elif escolha == "📁 CTRs Concluídas":
    st.title("📂 Histórico de CTRs Concluídas")
    df_c = carregar_dados("get_contracts"); df_i = carregar_dados("get_items"); df_m = carregar_dados("get_measurements")
    if not df_c.empty:
        df_done = df_c[df_c['status'] == 'Concluído']
        if st.session_state.usuario != "ADM": df_done = df_done[df_done['gestor'] == st.session_state.usuario]
        if df_done.empty: st.info("Nenhuma CTR concluída.")
        else:
            sel_hist = st.selectbox("Selecione para visualizar", df_done['ctt'].tolist())
            con = df_done[df_done['ctt'] == sel_hist].iloc[0]
            cid = con['contract_id']
            
            # --- ÁREA DE AÇÃO ADM (REABERTURA) ---
            if st.session_state.usuario == "ADM":
                st.warning("⚠️ Esta CTR está concluída. Deseja reativá-la para novos lançamentos?")
                if st.button("🔄 Reabrir Contrato (Voltar para Ativos)", use_container_width=True):
                    if salvar_dados_otimizado("contracts", {"status": "Ativo"}, "update", "id_field", "contract_id", "id_value", cid):
                        st.success(f"Contrato {con['ctt']} reativado com sucesso!")
                        st.rerun()
                st.divider()
            ...
            with st.container(border=True):
                st.subheader(f"🏁 Fechamento - {con['fornecedor']}")
                if not med_ctt.empty:
...
# --- 10. CONTRATOS (COM IMPORTAÇÃO PDF) ---
    ...
    if st.session_state.usuario != "ADM":
        ...
        with st.expander("📥 Importar Dados de PDF (Padrão CTT)"):
            uploaded_file = st.file_uploader("Arraste o PDF da CTT aqui", type="pdf")
        ...
        with st.form("f_con", clear_on_submit=True):
            ...
            gs = st.selectbox("Gestor", [k for k in USUARIOS.keys() if k != "ADM"])
            ...
            if st.form_submit_button("Salvar Contrato e Itens"):
                ...
                if salvar_dados_otimizado("contracts", {"contract_id": contract_id, "cliente": cl, "ctr": ctr, "fornecedor": fo, "ctt": ctt, "gestor": gs, "valor_contrato": vl, "data_inicio": str(dt_i), "data_fim": str(dt_f), "status": "Ativo"}):
                    itens_pdf = pdf_info.get("itens", [])
                    for item in itens_pdf:
                        salvar_dados_otimizado("items", {"item_id": str(uuid.uuid4()), "contract_id": contract_id, "descricao_item": item["desc"], "vlr_unit": item["valor"], "data_fim_item": str(dt_f)})
